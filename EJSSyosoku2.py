...
# ログインボタン
if st.button('Login'):
    if username == 'jrs' and password == 'jrs4352':  # この部分は実際の認証処理に置き換えてください
        st.success('Logged In Successfully')
        
        # ログイン成功後のメインページの内容を表示
        ...
        def main_page():
            ...
            # 今月の売上実績と予測金額
                ...
                col2.metric(f"💰{formatted2_date}までの売上実績", f"{actual_sales:,.0f}円")
                ...
                col4.metric(f"💰{formatted2_date}までの粗利実績", f"{actual_gross_profit:,.0f}円")
            ...
            # 達成度を計算
                ...
                # 売上実績と粗利実績のNaNを0に変換
                filtered_data['売上実績'] = filtered_data['売上実績'].fillna(0)
                filtered_data['粗利実績'] = filtered_data['粗利実績'].fillna(0)
                # 売上差額と粗利差額の計算
                filtered_data['売上差額'] = filtered_data['売上実績'] - filtered_data[f'EJSS売上{zenkaku_num(selected_month)}']
                filtered_data['粗利差額'] = filtered_data['粗利実績'] - filtered_data[f'EJSS粗利{zenkaku_num(selected_month)}']
